...
@section('content')
<div class="container">
	<h3>Editar Proveedores {{$proveedor->nombre}} <a href="{{route('proveedor.index','proveedor.indexcxp')}}"><button type="button" class="btn btn-success float-right">Regresar</button></a></h3><hr>
	@if ($errors->any())
	...
	<div class="row">
		<div class="col-sm">
		<form action="{{route('proveedor.update',[$proveedor->id,'proveedor.showcxp'])}}" method="POST" enctype="multipart/form-data">
			@method('PUT')
			@csrf
